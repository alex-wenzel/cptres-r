...
```{r fig.width=20}
hmrks <- run.fgsea("0", hallmarks)
react <- run.fgsea("0", reactome)

for (i in 1:8) {
  hmrks <- rbind(hmrks, run.fgsea(as.character(i), hallmarks))
}

for (i in 1:8) {
  react <- rbind(react, run.fgsea(as.character(i), reactome))
}
```
...
hmrks.hm <- pheatmap(hmrks.mat, silent=T, cluster_rows=F, cluster_cols=F)

react.mat <- acast(react, pathway~cluster, value.var="NES", fill=0)
react.hm <- pheatmap(react.mat, silent=T, cluster_rows=F, cluster_cols=F)
```

```{r}
plot(hmrks.hm$gtable)
```

```{r fig.width=15, fig.height=14}
plot(react.hm$gtable)
```
