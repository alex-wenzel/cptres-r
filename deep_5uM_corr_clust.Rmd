---
title: "deep_5uM Correlation/Clustering"
output: html_notebook
---

Original versions of this notebook tried various methods to create a cell by cell Spearman 
correlation matrix with the entire `deep_5uM` dataset without success mostly due to technical issues
of memory and processing time. Instead, 3 500-cell random subsets have been created from the original
dataset. Cell by cell correlation matrices are calculated separately and averaged for each.
...
### Load `deep_5uM` Subsets

```{r results="hide"}
sc.1 <- load.seurat("../deep_5uM/subset_500_1/")
sc.2 <- load.seurat("../deep_5uM/subset_500_2/")
sc.3 <- load.seurat("../deep_5uM/subset_500_3/")
```
...
### Cutree Clustering For `sc.3`

```{r}
var.anno.col.3 <- data.frame(
  Experiment=factor(var.exps.3)
)
var.ph.3.cut <- pheatmap(var.corr.mat.3, cluster_rows=T, cluster_cols=T,
                 show_colnames=F, show_rownames=F, silent=T,
                 annotation_col=var.anno.col.3, annotation_row=var.anno.col.3,
                 cutree_cols=7)
```
...
```{r}
sc.3.ph.clust <- as.factor(cutree(var.ph.3.cut$tree_col, k=7))
attributes(sc.3.ph.clust)$names <- sc.3@cell.names
sc.3@ident <- sc.3.ph.clust
```
...
```{r}
sc.3.exps.clusters <- data.frame(
  experiment=sapply(sc.3@cell.names, get.pop.name),
  cluster=sc.3@ident,
  stringsAsFactors = F
)
rownames(sc.3.exps.clusters) <- sc.3@cell.names

sc.3.clust.count <- ddply(sc.3.exps.clusters, .(sc.3.exps.clusters$cluster, sc.3.exps.clusters$experiment), nrow)
names(sc.3.clust.count) <- c("cluster", "experiment", "count")
sc.3.clust.count.mat <- acast(sc.3.clust.count, cluster~experiment, value.var="count", fill=0)
sc.3.clust.count.mat <- sc.3.clust.count.mat/colSums(sc.3.clust.count.mat)[col(sc.3.clust.count.mat)]
```

```{r}
sc.3.clust.exp.hm <- pheatmap(sc.3.clust.count.mat, cluster_rows=F, cluster_cols=T,
                              show_rownames=T, show_colnames=T, silent=T)
```

```{r}
plot(sc.3.clust.exp.hm$gtable)
```
